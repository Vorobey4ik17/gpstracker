...
Запуск:  pip install flask flask-socketio   (один раз)
         python server.py
Карта:   http://localhost:5000  (с телефона: адрес туннеля)
"""
...
def init_db():
    ...
    # миграция: добавляем колонку модели телефона, если её ещё нет
    cols = [r[1] for r in con.execute("pragma table_info(locations)")]
    if "device_model" not in cols:
        con.execute("alter table locations add column device_model text")
    con.execute(
...
def points():
    ...
    for r in rows:
        ...
        con.execute(
            "insert into locations(device_id,device_model,lat,lon,accuracy,speed,created_at)"
            " values(?,?,?,?,?,?,?)",
